...
echo $(( $(echo $($PSQL "SELECT SUM(winner_goals),SUM(opponent_goals) FROM games") | tr \| +) ))
...
echo "$($PSQL "SELECT (SUM(winner_goals)+SUM(opponent_goals)+0.0)/COUNT(*) FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams t LEFT JOIN games gw ON t.team_id=gw.winner_id LEFT JOIN games go on t.team_id=go.opponent_id WHERE (gw.year=2014 OR go.year=2014) AND (gw.round='Eighth-Final' OR go.round='Eighth-Final') ORDER BY name")"
